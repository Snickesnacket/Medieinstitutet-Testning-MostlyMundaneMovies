...
describe('Mostly Mundane Movies', () => {
	...
	context('Home page', () => {
		...
		it.only('Kan inte söka utan att skriva in en titel', () => {
			cy.get('input.form-control').clear().should('have.length.at.least', 3)
		})

		it('Kan inte söka utan att skriva in en titel', () => {
			cy.get('.formMovieSearch').submit()
			cy.get('#error').should('be.visible').contains('Wow, that was stupid /n Search query must be at least 3 characters long, duh  ')
		})
